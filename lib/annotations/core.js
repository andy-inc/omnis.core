...
var ignoreAnnotations = [
    'author', 'constructor', 'deprecated', 'exception', 'param', 'private', 'return', 'returns',
    'see', 'this', 'throws', 'version'
];
...
var linetypes = {
    ...
    fnsig7  : { pattern: /(?:module|define)\s*\(\s*(?:(?:'([^']+)')|(?:"([^"]+)")).*function/, returns: { name: '$module' } }
};
    ...
    for(var num in lines) if(lines.hasOwnProperty(num)) {
        ...
        switch(type.id) {
            ...
            case 'dbanno':
                ...
                if (ignoreAnnotations.indexOf(type.name) === -1){
