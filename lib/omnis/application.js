...
var annotations = require('../annotations'),
    ...
    NS = require('./namespaces').Namespaces;
...
var Application = function(name, omnis){
    ...
    $glob.sync('**/*.js', {cwd: $path.join(__dirname, 'modules') }).forEach(function(el){
        self._toDeclare.push(require($path.join(__dirname, 'modules', el)));
    });
};

Application.prototype.init = function(config){
    this._config = $extend(true, this._config, config);
    this._ns.setDebug(config.debug);
    return this._declareAll();
};
...
Application.prototype.setup = function(){
    ...
    return $q().then(function(){
        ...
        self._ns.getAll().map(function(el){ return self._ns.get(el); }).forEach(function(module){
...
Application.prototype.module = function(namespace, name, filename, fn){
...
Application.prototype.inject = function(ns, fn){
    ...
    var self = this,
        allModulesNames = this._toDeclare.map(function(el){
            return (el.ns || self._ns.default) + '.' + el.name;
        }),
        params;
    if (Array.isArray(fn)){
        params = [].concat(fn);
    ...
    for(var i = 0; i < params.length; i++){
        var param = params[i];
        if (param.substr(0, 1) === "$"){
            var sysparam = self._ns.default + '.' + param;
            if (self._ns.get(sysparam)){
                args.push(self._ns.get(sysparam));
            } else {
                args.push(require(param.substr(1)));
            }
            continue;
        }
        //var depInPackage = self._ns.get(ns + '.' + param);
        var dep = null;
        if (param.indexOf('.') == -1) {
            dep = self._ns.get(ns + '.' + param);
            if (dep == null){
                dep = self._ns.get(self._ns.default + '.' + param);
            }
        } else {
            dep = self._ns.get(param)
        }
        if (!dep){
            if (allModulesNames.indexOf(param) == -1 && allModulesNames.indexOf(ns + '.' +param) == -1 && allModulesNames.indexOf(self._ns.default + '.' +param) == -1){
                throw new Error("Unknown dependence: " + param);
            }
            return null;
        } else {
            args.push(dep);
...
Application.prototype._declareAll = function(){
    ...
    return this.mapSeries(this._toDeclare, this._declare.bind(this)).then(function(){
...
Application.prototype._declare = function(module, allowError){
    ...
    return $q.spread([], function(){
        ...
        if (type == "module"){
            ...
            self._ns.register('$' + name, result);
        } else if (type === "annotation"){
            if (self._config.debug){
                console.log("Register annotation: " + name);
            }
